...
float Scene::renderingInconsolataFonts(char c, glm::vec2 offset, std::vector<Geometry> &objects) {
	GlyphExtractor g;
	g.LoadFontFile("fonts/inconsolata/Inconsolata.otf");
	MyGlyph a = g.ExtractGlyph(c);
	for (unsigned int i = 0; i < a.contours.size(); i++) {
		//std::cout << "Contour " << i << std::endl;
		for (unsigned int j = 0; j < a.contours[i].size(); j++) {
...
float Scene::renderingGreatVibeFonts(char c, glm::vec2 offset, std::vector<Geometry> &objects) {
	GlyphExtractor g;
...
float Scene::setInconsolataText(std::string s, std::vector<Geometry> &objects) {
	objects.clear();
	spiral.verts.clear();
	line.verts.clear();
	int str_off = s.size() / 5;
	...
	for (unsigned int i = 0; i < s.size(); i++)
	{
		offsetX = renderingInconsolataFonts(s[i], testing, objects);
		testing += glm::vec2(offsetX, 0);
		textLength += offsetX;
	}
	char space = '1';
...
float Scene::setGreatVibeText(std::string s, std::vector<Geometry> &objects) {
	objects.clear();
	spiral.verts.clear();
	line.verts.clear();
	int str_off = s.size() / 5;
	...
	for (unsigned int i = 0; i < s.size(); i++)
	{
		offsetX = renderingGreatVibeFonts(s[i], testing, objects);
		testing += glm::vec2(offsetX, 0);
		textLength += offsetX;
	}
	char space = '1';
